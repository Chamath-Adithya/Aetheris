...
// Veritas Master Orchestrator — runs all intelligence sources in parallel
...
export async function fullBriefing() {
  console.error('[Veritas] Starting intelligence sweep — 29 sources...');
  const start = Date.now();
  ...
  console.error(`[Veritas] Sweep complete in ${totalMs}ms — ${output.veritas.sourcesOk}/${sources.length} sources returned data`);
  return output;
}
