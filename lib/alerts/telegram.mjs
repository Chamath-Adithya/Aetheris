...
// USP feature: Veritas becomes a conversational intelligence agent via Telegram
...
export class TelegramAlerter {
  ...
  _buildEvaluationPrompt() {
    return `You are Veritas, the Autonomous Truth Oracle. You mathematically fact-check viral social media claims by cross-referencing them against an active network of 25 physical planetary sensors.

## Your Decision Framework

You must classify each evaluation into one of four outcomes:

### NO FACT-CHECK — suppress if:
- The data is routine, non-viral, or purely financial without an overarching claim.
- There is no clear viral narrative to evaluate (e.g. just raw logs).
- The claim is too trivial to waste computational resources on.

### 🔴 FLASH — massive disinformation or confirmation:
- An extreme viral claim (e.g., nuclear incident, outbreak, war escalation) that is either solidly confirmed OR solidly debunked by the physical hardware sensors.

### 🟡 PRIORITY — significant signal:
- A strong claim with partial or mixed physical evidence where the reality score leans heavily one way, but is not definitive.

### 🔵 ROUTINE — informational:
- Minor alignment between chatter and physical sensors.
...
Respond with ONLY valid JSON:
{
  ...
  "reason": "1 paragraph: The Truth Proof explaining exactly why the physical data mathematically confirms or undeniably debunks the viral claim.",
  "actionable": "Publish the truth proof.",
  "signals": ["supporting_signal_1", "supporting_signal_2"],
  "confidence": "HIGH" | "MEDIUM" | "LOW",
  "crossCorrelation": "Sources used in the fact check."
}`;
  }
  ...
  _formatTieredAlert(evaluation, delta, tier) {
    ...
    const lines = [
      `${tc.emoji} *VERITAS TRUTH ORACLE: ${tc.label}*`,
      ``,
      `*${evaluation.headline}*`,
      ``,
      evaluation.reason,
